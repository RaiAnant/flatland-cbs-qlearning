...
class MotionCheck(object):
    ...
    def find_conflicts(self, env, actions, contenders = None):
        ...
        # svStops = set() #CHANGED: remove this line for original code
        ...
        for (v, dPred) in self.G.pred.items():
            ...
            # not blocked but has two or more predecessors, ie >=2 agents waiting to enter this node
                ...
                if v in dPred:
                    stoppedAgent = self.G.nodes[v].get("agent");
                    diAgCell.pop(stoppedAgent)
                    poplist = []
                    for k in diAgCell:
                        agent = env.agents[k]
                        next_pos, next_dir = env_utils.apply_action_independent(RailEnvActions(actions[k]),
                                                                                env.rail,
                                                                                agent.position,
                                                                                agent.direction)
                        if next_dir!=env.agents[stoppedAgent].direction:
                            poplist.append(k)
                    for a in poplist:
                        diAgCell.pop(a)
                else:
                    iAgWinner = min(diAgCell)
                    if contenders is not None and diAgCell[iAgWinner][0]>0:
                        contenders.append(list(diAgCell.keys()))
    ...
    def check_motion(self, iAgent, rcPos, mode = 2):
        ...
        if "color" in dAttr:
            sColor = dAttr["color"]
            if sColor == "red" or (sColor=="purple" and mode==0): #, "purple" ]:  #CHANGED
                return sColor, False
