...
class RailEnv(Environment):
    ...
    def step(self, action_dict_: Dict[int, RailEnvActions], cords_map={}, contenders = [], mode = 2):
        """
        ...
        self.position_agent_map = {} #CHANGED: line added
        for agent in self.agents:
            i_agent = agent.handle
            if agent.position not in self.position_agent_map: self.position_agent_map[agent.position]=[]
            self.position_agent_map[agent.position].append(i_agent)
        ...
        self.motionCheck.find_conflicts(self, action_dict_, contenders)
        ...
        for agent in self.agents:
            ...
            if agent.malfunction_handler.in_malfunction:
                ...
                color, movement_allowed = self.motionCheck.check_motion(i_agent, agent.position, mode)

                # if color== "red":
                #     next_pos, next_dir = env_utils.apply_action_independent(RailEnvActions(actions[i_agent]),
                #                                                      env.rail,
                #                                                      agent.position,
                #                                                      agent.direction)
                #     if next_pos in self.position_agent_map:
                #
                #         next_agent = None
                #         for i_ag in self.position_agent_map[next_pos]:
                #             ag = self.agents[i_ag]
                #             if ag.direction==next_dir:
                #                 next_agent = ag
                #                 break
                #
                #         current_agent = agent
                #         while next_agent.direction == next_dir:
                #             next_pos, next_dir = env_utils.apply_action_independent(RailEnvActions(actions[next_agent.handle]),
                #                                                              env.rail,
                #                                                              next_agent.position,
                #                                                              next_agent.direction)
                #             if next_pos not in self.position_agent_map: break
                #             temp = None
                #             for ag in self.agents[self.position_agent_map[next_pos]]:
                #                 if ag.direction == next_dir:
                #                     temp = ag
                #                     break
                ...
                #             next_agent = temp
                #         if next_agent.direction == next_dir:


                if color=="purple":
                    if agent.position not in cords_map:
                        cords_map[agent.position] = [agent.handle]
                    else:
                        cords_map[agent.position].append(agent.handle)

                if mode == 1:
                    movement_allowed = True
                # movement_allowed = True # CHANGED: self.motionCheck.check_motion(i_agent, agent.position)
